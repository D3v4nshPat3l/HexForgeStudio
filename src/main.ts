...
function setCursor(offset: number, extendSelection = false, reveal = true): void {
  ...
  if (reveal) {
    ...
    tab.hexScrollTop = Math.max(0, hexRowToTop(row, totalRows, height) - HEX_ROW_HEIGHT * 4);
  }
  updateAll();
...
function virtualHexHeight(totalRows: number): number {
  return Math.max(1, Math.min(MAX_HEX_SCROLL_HEIGHT, totalRows * HEX_ROW_HEIGHT));
}
...
function hexScrollTopToRow(scrollTop: number, totalRows: number, height: number, viewportHeight: number): number {
  ...
  if (totalRows * HEX_ROW_HEIGHT <= MAX_HEX_SCROLL_HEIGHT) return Math.floor(scrollTop / HEX_ROW_HEIGHT);
...
function hexRowToTop(row: number, totalRows: number, height: number): number {
  if (totalRows <= 1) return 0;
  if (totalRows * HEX_ROW_HEIGHT <= MAX_HEX_SCROLL_HEIGHT) return row * HEX_ROW_HEIGHT;
  return (row / Math.max(1, totalRows - 1)) * Math.max(0, height - HEX_ROW_HEIGHT);
}
...
function renderHexView(): void {
  ...
  if (!grid || !header || !topScroll) return;
...
async function renderHexRows(tab: EditorTab): Promise<void> {
  ...
  const visibleRows = Math.ceil(Math.max(400, grid.clientHeight) / HEX_ROW_HEIGHT) + HEX_OVERSCAN_ROWS * 2;
...
app.addEventListener("keydown", (event) => {
  ...
  if (event.key in moves) { event.preventDefault(); setCursor(tab.cursor + (moves[event.key] ?? 0), event.shiftKey); return; }
  if (event.key === "Home") { event.preventDefault(); setCursor(Math.floor(tab.cursor / bytesPerRow) * bytesPerRow, event.shiftKey); return; }
  if (event.key === "End") { event.preventDefault(); setCursor(Math.min(tab.file.size - 1, Math.floor(tab.cursor / bytesPerRow) * bytesPerRow + bytesPerRow - 1), event.shiftKey); return; }
  if (event.ctrlKey || event.metaKey || event.altKey) return;
  if (tab.inputMode === "hex" && /^[0-9a-f]$/i.test(event.key)) {
    event.preventDefault();
    const nibbleValue = Number.parseInt(event.key, 16);
    editQueue = editQueue.then(async () => {
      const current = await readByte(tab, tab.cursor);
      const before = tab.patches.get(tab.cursor) ?? null;
      if (tab.nibble === 0) {
        const after = (nibbleValue << 4) | (current & 0x0F);
        tab.patches.set(tab.cursor, after);
        recordPatch(tab, "Edit high nibble", [{ offset: tab.cursor, before, after }]);
        tab.nibble = 1;
        updateAll();
      } else {
        const after = (current & 0xF0) | nibbleValue;
        tab.patches.set(tab.cursor, after);
        recordPatch(tab, "Edit low nibble", [{ offset: tab.cursor, before, after }]);
        tab.nibble = 0;
        setCursor(tab.cursor + 1);
      }
    });
  } else if (tab.inputMode === "text" && event.key.length === 1) {
    const code = event.key.charCodeAt(0);
    if (code <= 0xFF) { event.preventDefault(); setBytes(tab, tab.cursor, new Uint8Array([code]), "Edit text byte"); setCursor(tab.cursor + 1); }
  }
});
...
fileInput.addEventListener("change", () => { openFiles(Array.from(fileInput.files ?? [])); fileInput.value = ""; });
compareInput.addEventListener("change", () => { const file = compareInput.files?.[0]; if (file) void compareWith(file); compareInput.value = ""; });
